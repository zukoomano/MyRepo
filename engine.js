...
function generateSignals(candles, opts) {
  ...
  // ── HARDENING 1: ANTI-CHOP / WHIPSAW FILTER ──
  // Efficiency ratio: how much price actually travelled vs how much it wandered.
  // Low ratio = choppy noise (the #1 killer of indicator signals). Refuse to trade it.
  const ER_LOOK = 20;
  let netMove = 0, pathLen = 0;
  if (n > ER_LOOK) {
    netMove = Math.abs(closes[n-1] - closes[n-1-ER_LOOK]);
    for (let i = n-ER_LOOK; i < n; i++) pathLen += Math.abs(closes[i] - closes[i-1]);
  }
  const efficiency = pathLen > 0 ? netMove / pathLen : 0;   // 0 = pure chop, 1 = clean directional
  const isChop = efficiency < 0.15;   // only veto genuinely directionless noise

  // ── HARDENING 2: VOLATILITY REGIME ──
  // Compare current ATR to its own recent average. Extreme vol = spreads widen, stops get hunted.
  const atrArrFull = calcATR(highs, lows, closes);
  const atrNow = atrArrFull[n-1] || 0;
  const atrRecent = atrArrFull.slice(-50).filter(v=>v!=null);
  const atrAvg = atrRecent.length ? atrRecent.reduce((a,b)=>a+b,0)/atrRecent.length : atrNow;
  const volRatio = atrAvg > 0 ? atrNow/atrAvg : 1;
  const volShock = volRatio > 2.0;        // volatility spike — news/panic, stand aside
  const volDead  = volRatio < 0.5;        // dead market — no follow-through

  const regime = (struct.trend!=="NEUTRAL" && !squeeze && strongTrend && !isChop) ? "TREND" : "RANGE";
  ...
  // QUALITY DEFINITION (stricter than before, but still fires on real setups):
  //   Every signal needs LOCATION + TRIGGER (the core), PLUS one strong confirmation:
  ...
  let overall="NEUTRAL", conf=0;
  if(buyReady && !sellReady){ overall="BUY";  conf=Math.min(95,B.sc); }
  else if(sellReady && !buyReady){ overall="SELL"; conf=Math.min(95,S.sc); }
  if(overall==="NEUTRAL") conf=Math.max(B.sc,S.sc);

  // ── HARDENING 3: VETO LAYER ──
  // Even a "valid" setup gets killed if the market conditions make it a bad bet.
  // This is the part most retail tools skip — knowing when NOT to trust your own signal.
  const vetoes = [];
  if (isChop)   vetoes.push(`Choppy market (efficiency ${(efficiency*100).toFixed(0)}% — price is wandering, not trending)`);
  if (volShock) vetoes.push(`Volatility spike (${volRatio.toFixed(1)}× normal — spreads widen, stops get hunted)`);
  if (volDead)  vetoes.push(`Dead market (${volRatio.toFixed(1)}× normal — no follow-through likely)`);

  // Spread cost: reject if the broker's spread eats too much of the target
  const spreadPts = opts.spreadPoints != null ? opts.spreadPoints : 0;
  const slPtsCfg  = opts.slPoints != null ? opts.slPoints : 20;
  const targetPts = slPtsCfg * 2;                                   // 2:1 target
  const spreadCostPct = targetPts > 0 ? (spreadPts / targetPts) * 100 : 0;
  if (spreadPts > 0 && spreadCostPct > 15) {
    vetoes.push(`Spread too costly (${spreadPts} pts = ${spreadCostPct.toFixed(0)}% of your target)`);
  }

  if (vetoes.length && overall !== "NEUTRAL") {
    overall = "NEUTRAL";
  ...
  const strategies=[
    ...
    gateRow("⑥ Quality", "NEUTRAL", vetoes.length ? ("⛔ VETOED — "+vetoes[0]) : `Clean conditions · efficiency ${(efficiency*100).toFixed(0)}% · vol ${volRatio.toFixed(1)}×`, "🛡"),
    ...reads,
  ];
  ...
  // ===== Targets: structure-based stop, clamped, with a clean 2:1 (TP1) and 3:1 (TP2) =====
  ...
  const RR_TARGET = 2;                       // reward:risk enforced on every signal
  // ── HARDENING 4: VOLATILITY-ADAPTIVE STOP ──
  // A fixed stop is too tight in fast markets, too wide in quiet ones.
  // Anchor the stop to structure, but require at least 1.2×ATR of breathing room
  // so normal noise doesn't stop you out. Your point setting acts as the floor.
  const atrStop = atrNow * 1.2;
  ...
  const minDist = Math.max(slPts*pointSize, atrStop);   // floor = max(your setting, 1.2×ATR)
  const maxDist = Math.max(slPts*pointSize*2.5, atrStop*2);  // ceiling keeps the 2:1 target reachable
  let sl,tp1,tp2;
  if(overall==="BUY"){
    let d = (price - (lastSwingLow - buffer));
    d = Math.max(minDist, Math.min(maxDist, d));
    sl  = price - d;
    tp1 = price + RR_TARGET*d;               // exactly 2:1
    tp2 = price + (RR_TARGET+1)*d;            // 3:1 runner
  } else if(overall==="SELL"){
    let d = ((lastSwingHigh + buffer) - price);
    d = Math.max(minDist, Math.min(maxDist, d));
    sl  = price + d;
    tp1 = price - RR_TARGET*d;
    tp2 = price - (RR_TARGET+1)*d;
  }
  ...
  return {overall,conf,buys,sells,strategies,
    ...
    fib,atr,price,squeeze,regime,htf,vetoes,efficiency,volRatio,isChop,volShock,timestamp:new Date()};
}
